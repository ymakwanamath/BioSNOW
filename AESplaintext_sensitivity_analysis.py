...
if __name__== "__main__":
    ...
    for i in range(len(Z)):
        print(Z[i])
    streamA=Z.copy()
    flat_streamA=convert_to_binary_array(flatten_list(streamA))
    plaintext1=[randint(0,1) for __ in range(len(flat_streamA))]
    plaintext2=flip_one_bit(plaintext1)
    ciphertext1=xor_arrays(plaintext1, flat_streamA)
    Z=[]
    IV=[]
    #IV=generate_key(128)
    ...
    keystream_generate(5000)
    streamB=Z.copy()
    flat_streamB=convert_to_binary_array(flatten_list(streamB))
    ciphertext2=xor_arrays(plaintext2, flat_streamB)
    hd=hamming_distance(ciphertext1, ciphertext2)
    lencipher=len(ciphertext1)

    print("Avalanche Effect for plaintext sensitivity is ")
    print(hd/lencipher)
...
'''
    ...
    keystream_generate(100)
    for i in range(len(Z)):
        print(Z[i])
    streamB=Z.copy()
    flat_streamB=flatten_list(streamB)
    ciphertext2=xor_arrays(flat_streamB, plaintext2)
'''    
