...
def generate_chacha_keystream(key, nonce, length):
    """
    Generate keystream bytes of specified length using ChaCha20.
    ...
    """
    # Create ChaCha20 cipher instance, starting from counter 0
    algorithm = algorithms.ChaCha20(key, nonce)
    cipher = Cipher(algorithm, mode=None, backend=default_backend())

    encryptor = cipher.encryptor()

    # Encrypt zero bytes to produce keystream
    keystream = encryptor.update(b'\x00' * length)

    return keystream


def aes_keystream(key, nbytes):
    nonce = get_random_bytes(16)  # 128-bit nonce
    cipher = Cipher(algorithms.AES(key), modes.CTR(nonce), backend=default_backend())
    encryptor = cipher.encryptor()
    keystream = encryptor.update(b'\x00' * nbytes)
    return nonce, keystream

# Generate RC4 keystream bytes of specified length
def rc4_keystream(key, nbytes):
    cipher = ARC4.new(key)
    keystream = cipher.encrypt(b'\x00' * nbytes)
    return keystream
...
if __name__== "__main__":
    ...
    K=generate_integer_key(128)
    IV=generate_integer_key(128)
    #for __ in range(128):
    ...
    keystream_generate(62)
    end_biosnow=time.time()    

    start_chacha=time.time()
    key = b'0123456789abcdef0123456789abcdef'  # 32-byte key
    nonce = b'abcdef012345abcd'  # 12-byte nonce

    length =  1000 # number of keystream bytes wanted
    ks = generate_chacha_keystream(key, nonce, length)
    end_chacha=time.time()


    nbytes = 1000 # number of keystream bytes to generate
    start_aes=time.time()
    # AES key (16, 24 or 32 bytes)
    aes_key = get_random_bytes(32)
    nonce, aes_stream = aes_keystream(aes_key, nbytes)
    #print(f"AES Keystream (nonce + keystream) hex:\n{nonce.hex()} {aes_stream.hex()}")
    
    end_aes=time.time()

    start_rc4=time.time()
    # RC4 key (5 to 256 bytes)
    rc4_key = get_random_bytes(16)
    rc4_stream = rc4_keystream(rc4_key, nbytes)
    #print(f"RC4 Keystream hex:\n{rc4_stream.hex()}")
    end_rc4=time.time()

    print("time taken by biosnow")
    print(end_biosnow-start_biosnow)
    print("time taken by chacha")
    print(end_chacha-start_chacha)

    print("time taken by AES")
    print(end_aes-start_aes)

    print("time taken by rc4")
    print(end_rc4-start_rc4)


    print("speed of rc4 is \n")
    print( (8*nbytes) / ((end_rc4-start_rc4)*1024*1024))

    print("speed of aes is \n")
    print( (8*nbytes) / ((end_aes-start_aes)*1024*1024))
    
    
    print("speed of chacha is \n")
    print( (8*nbytes) / ((end_chacha - start_chacha)*1024*1024))

    
    print("speed of biosnow  is \n")
    print( (8*nbytes) / ((end_biosnow- start_biosnow)*1024*1024))
